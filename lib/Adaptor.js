...
const query = (0, _lodashFp.curry)(function (qs, callback, state) {
  let {
    connection
  } = state;
  ...
  return connection.query(qs, function (err, result) {
    ...
    console.log('Results retrieved and pushed to position [0] of the references array.');
...
const bulk = (0, _lodashFp.curry)(function (sObject, operation, options, fun, state) {
  ...
  if (finalAttrs.length > 10000) console.log('Your batch is bigger than 10,000 records; chunking...');
  const chunkedBatches = (0, _languageCommon.chunk)(finalAttrs, 10000);
  return Promise.all(chunkedBatches.map(chunkedBatch => new Promise((resolve, reject) => {
    const timeout = pollTimeout || 240000;
    const interval = pollInterval || 6000;
    console.info(`Creating bulk ${operation} job for ${sObject} with ${chunkedBatch.length} records`);
    ...
    return batch.on('queue', function (batchInfo) {
      ...
      errors.forEach(err => {
        err[`${options.extIdField}`] = chunkedBatch[err.position - 1][options.extIdField];
      });
    ...
    return { ...state,
      references: [merged, ...state.references]
    };
  });
});
/**
