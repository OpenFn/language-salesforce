...
const bulk = (0, _lodashFp.curry)(function (sObject, operation, options, fun, state) {
  ...
  console.info(`Creating bulk ${operation} job for ${sObject}`, finalAttrs); // console.info('Creating batch for job.');
      ...
      if (res) {
        for (var i = 0; i < res.length; i++) {
          if (res[i].success) {
            ...
            console.log(`#${i + 1} error occurred, message = ${JSON.stringify(res[i], null, 2)}`);
            reject();
          }
        }
      }
